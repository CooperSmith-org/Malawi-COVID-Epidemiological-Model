...
execute_stepwise <- function(inputs, masking_path, distancing_path, param_row) {
  ...
  for (age in ages) {
    dfs_ages[[age]]$pop_infection_rate <- base_infection_rate *
      susceptibility[[age]] / dfs_ages[[age]]$Population
  ...
  e[[AGE_ADULT]][, 1] <- 1 ## Hopefully creates intended starting condition
