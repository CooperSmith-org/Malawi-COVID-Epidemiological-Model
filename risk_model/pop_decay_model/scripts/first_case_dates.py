...
ADJACENCY_MATRIX = '../adj_matrix_granular.csv'
FIRST_CASES_DATA = "../static/initial_seeds.csv"
TA_NAME_MAP = "../static/ta_name_map.csv"
TA_LVL_MAP = "../static/ta_lvl_map.csv"
LVLS_MAP = "../static/lvlmap.csv"
...
def calculate_dates(adj_matrix, first_case_data, offset):
	...
	start_date_list = []
	# print(first_case_data)

	### calculate offset
	for row in first_case_data.itertuples():
		start_dates = adj_matrix[row.TA_Code].apply(lambda x: offset_date(row.Date, offset, x))
	...
	first_dates = first_dates.merge(first_case_data[['Date', 'TA_Code']], left_on='ADM3_PCODE', right_on='TA_Code', how='left')
	first_dates.loc[first_dates['TA_Code'].notnull(), 'first_case_date'] = \
		first_dates.loc[first_dates['TA_Code'].notnull(), 'Date']
	first_dates.drop(['TA_Code', 'Date'], axis=1, inplace=True)
...
def main():
	...
	mw_shape = prep_shape(SHAPE_PATH)[['ADM2_PCODE', 'ADM3_PCODE', 'ADM3_EN', 'ADM2_EN']]

	adj_matrix.fillna(1000, inplace=True)
	first_case_data = pd.read_csv(FIRST_CASES_DATA, parse_dates=['Date'])
	first_case_data['TA_Code'] = first_case_data['TA_Code'].apply(lambda x: "MW" + str(x))
	# first_case_data.replace(REPLACEMENTS, inplace=True)

	# return first_case_data
	names = pd.read_csv(TA_LVL_MAP)
	
	# return calculate_dates(adj_matrix, first_case_data, 20)
	...
	for offset in range(0, 51):
		...
		first_dates.to_csv(join(OUTPATH, 'seeds_offset_param_{}.csv'.format(offset)), index=False)


def prep_shape(shape_path):

	mw = gpd.read_file(shape_path)
	mw = mw[['DIST_NAME', 'DIST_CODE', 'TA_NAME', 'TA_CODE', 'geometry']]
...
def execute_adj_matrix(degree):
	...
	mw = prep_shape(SHAPE_PATH)
	adj_dict = create_adj_dict(mw)
	adj_matrix = create_adj_matrix(adj_dict, degree)
	adj_matrix.to_csv("../adj_matrix_granular.csv")
